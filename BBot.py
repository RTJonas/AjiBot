...
from BinanceTudor import Binance
...
class TradingModel:
    ...
    def __init__(self, symbol):
        ...
        try:
            ...
            print("Exception ocurred while trying to print indicators on "+self.symbol)
            print(e)
            return None             
    ...
    #''' If price is 10% below the SMA puts a BuyOrder '''
    ...
    #''' If price is %5 below the LowerBollingerBand put a BuyOrder '''
        
        df = self.df
        buy_price = df['low_boll'][i]
        if buy_price >= df['close'][i]: #CHanged >= for <=
            self.buy_signals.append([df['time'][i],df['close'][i], df['close'][i]*1.045])
            return True
        return False
...
def Main():
    ...
    for symbol in symbols:
        ...
        plot = False
         ...
         #   print("Strategy worked on"+symbol)
          #  model.plotData()
           # plot = True
